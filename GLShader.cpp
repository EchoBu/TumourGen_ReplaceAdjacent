...
void GLShader::SetUniform( const char *name, const iv::vec3 & v )
{
...
void GLShader::SetUniform( const char *name, const iv::vec4 & v )
{
	int loc = GetUniformLocation(name);
	if( loc >= 0 ) {
...
void GLShader::SetUniform( const char *name, const iv::vec2 & v )
{
	int loc = GetUniformLocation(name);
	if( loc >= 0 ) {
...
void GLShader::SetUniform( const char *name, const iv::mat4 & m )
{
	int loc = GetUniformLocation(name);
	if( loc >= 0 )
	{
...
void GLShader::SetUniform( const char *name, const iv::mat3 & m )
{
	int loc = GetUniformLocation(name);
	if( loc >= 0 )
	{
...
{
	if( ! CompileShaderFromString(src0,type0) )
	{
		printf("Vertex shader failed to compile!\n%s", GetLog().c_str());
		getchar();
		exit(1);
	}
	if( ! CompileShaderFromString(src1,type1) )
	{
		printf("Geometry shader failed to compile!\n%s",GetLog().c_str());
		getchar();
		exit(1);
	}
	if( ! CompileShaderFromString(src2,type2) )
	{
		printf("Fragment shader failed to compile!\n%s",GetLog().c_str());
		getchar();
		exit(1);
...
bool GLShader::LoadFromString( const std::string &src0,GLSLShader::GLSLShaderType type0, const std::string &src1,GLSLShader::GLSLShaderType type1 )
{
	if( ! CompileShaderFromString(src0,type0) )
	{
		printf("Vertex shader failed to compile!\n%s", GetLog().c_str());
