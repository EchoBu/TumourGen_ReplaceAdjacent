...
namespace iv
{
	...
	template<typename Type>
	Matrix4<Type> ortho(Type _left,Type _right,Type _top,Type _bottom,Type _near,Type _far)
	{
		Matrix4<Type> mat((Type)0);
		Type rxRange = 1.0f / (_right - _left);
		Type ryRange = 1.0f / (_top - _bottom);
		Type rzRange = 1.0f / (_far - _near);
		mat.v[0] = rxRange * 2.0f;
		mat.v[5] = ryRange * 2.0f;
		mat.v[10] = -2.0f * rzRange;
		mat.v[12] = -1.0f * (_right + _left) * rxRange;
		mat.v[13] = -1.0f * (_top + _bottom) * ryRange;
		mat.v[14] = -1.0f * (_far + _near) * rzRange;
		mat.v[15] = 1.0f;
		return mat;
	}
	...
	template<typename T>
	Vector3<T> unproject(const Matrix4<T>& pMat,const Matrix4<T>& vMat,const Matrix4<T>& mMat,const Vector4<T>& vp,const Vector3<T>& src)
	{
		Matrix4<T> MVP = pMat * vMat * mMat;
		Matrix4<T> invMVP = inverse(MVP);
		Vector4<T> in,out;
		in.v[0] = ((src.v[0] - vp.v[0])/vp.v[2]) * (T)2 -(T)1;
		in.v[1] = ((src.v[1] - vp.v[1])/vp.v[3]) * (T)2 -(T)1;
		in.v[2] = (T)2 * src.v[2]-(T)1;
		in.v[3] = (T)1;
		out = invMVP * in;
		if( IsZero(out.v[3]))
			return Vector3<T>((T)0);
		out.v[3] = (T)1 / out.v[3];
		return Vector3<T>( out.v[0] * out.v[3],
			out.v[1]*out.v[3],
			out.v[2]*out.v[3]);
	}
	...
	template<typename Type>
	Vector3<Type> Projection( const Vector3<Type>& v,const Vector3<Type>& x,const Vector3<Type>& y )
	{
		return x * dot(v,x) + y * dot(v,y);
	}

	template<typename Type>
	Type GetPreciseAngle( const Vector3<Type>& v,const Vector3<Type>& x, const Vector3<Type>& y )
	{
		Type res = 0.0;
		if( dot(v,x) > 0.0 )
